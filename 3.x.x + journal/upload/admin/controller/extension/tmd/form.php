...
/// Product		
	...
	public function export(){
		...
		$form_infos=$this->model_extension_tmd_form->getexportheading($this->request->get['form_id']);
		...
		if(isset($form_infos))
		{
		 ...
		 $cel++;
		 $objPHPExcel->getActiveSheet()->SetCellValue($cel.$i, 'Total');
		 $cel++;
		 $objPHPExcel->getActiveSheet()->SetCellValue($cel.$i, 'Status');
		 $cel++;
		...
		$submitdatas=$this->model_extension_tmd_form->getexportsubmit($this->request->get['form_id']);
		...
		if(isset($submitdatas)) {			
			foreach($submitdatas as $submitdata) {
				...
				$value=$this->model_extension_tmd_form->getFieldExport($submitdata['fs_id'],$this->request->get['form_id'],$form_info['field_id']);
				...
				$objPHPExcel->getActiveSheet()->SetCellValue($cel.$i, $total);
				$cel++;
				$objPHPExcel->getActiveSheet()->SetCellValue($cel.$i, $paid_status ? $this->language->get('text_paid') : $this->language->get('text_unpaid'));
