<?php
class ControllerExtensionTMDKeysubmit  extends Controller {
	private $error = array();
	public function loadfrom() {
		
		$user_token= $this->session->data['user_token'];
		$regkey= $this->config->get('moduledata_formbuilder_key');
	...
	public function index() {
		...
		$this->load->language('extension/tmd/keysubmit');
		...
		curl_setopt($curl, CURLOPT_POSTFIELDS, http_build_query($tmd_extensiondata, '', '&'));
		 ...
		 $status=json_decode($response,true);
			if($status['status']) {
				$keydata=array();
				$keydata['moduledata_formbuilder_key']=$this->request->post['moduledata_key'];
