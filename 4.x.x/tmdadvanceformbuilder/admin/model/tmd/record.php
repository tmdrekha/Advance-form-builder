...
class Record extends \Opencart\System\Engine\Model {
	...
	public function getTmdlangaugeRecord($fs_id){
		
		$sql = "SELECT ts.language_id FROM " . DB_PREFIX . "tmdform_submit ts LEFT JOIN " . DB_PREFIX . "tmdform_description td ON (ts.form_id = td.form_id) where ts.fs_id='".$fs_id."' ";
		$query = $this->db->query($sql);
		return $query->row;
	}
